...
const DWORD CHyperLink::StyleCopyMenu	      = 0x00000080;		// No active color bit
...
{
	if (BITSET(m_dwStyle,StyleCopyMenu))
	{
	 ...
	 m_bMenuActive = TRUE;
		...
		{
			M.InsertMenu( 0xFFFFFFFF, MF_BYPOSITION,1, StringFromResource(IDS_HYPERLINK_COPY_ADDRESS) );
			M.InsertMenu( 0xFFFFFFFF, MF_BYPOSITION,2, StringFromResource(IDS_HYPERLINK_OPEN_ADDRESS) );
			...
			HCURSOR curArrow; 
			curArrow = ::LoadCursor(NULL, IDC_ARROW);
			if (curArrow) ::SetCursor(curArrow);
   //

			// track show popup menu
			int sel = M.TrackPopupMenu(TPM_NONOTIFY | TPM_RETURNCMD |TPM_LEFTALIGN   ,p.x,p.y,this);
			
			// change cursor back
			if (curArrow) ::DestroyCursor(curArrow);
			if (g_hLinkCursor) ::SetCursor(g_hLinkCursor);
			//

			switch(sel)
				{
				case 1:
					NTextClipboard::SetClipboardText(m_strURL);
				 break;
				case 2:
					FollowLink();
					break;
...
void CHyperLink::ReportError(int nError)
{
	...
	*/
	...
	AfxMessageBox( GetErrorStr(nError) , MB_ICONEXCLAMATION | MB_OK);
}
...
HINSTANCE CHyperLink::GotoURL(LPCTSTR url, int showcmd)
{
	...
	return ShellExecute(NULL, NULL, url, NULL, NULL, showcmd);
